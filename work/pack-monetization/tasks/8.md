...
Delete — this is a UI view task. Verification is via build success and visual inspection. Unit-testable logic (StoryInfo, LabyrinthLoader.loadStories) is covered in task 4's tests. BookshelfView's integration test (navigation flow) will be covered in task 11's test suite.
...
- [ ] NOTE: BookshelfView.swift is NOT yet in pbxproj — that happens in task 11. You must still create the file; just be aware the build will need manual Xcode addition or task 11's pbxproj update to fully compile.
