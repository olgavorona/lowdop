...
**Story-complete detection:** Check `labyrinth.difficulty == "hard"` since there are exactly 3 levels per story (easy/medium/hard) and hard is always the last. This avoids needing to query the manifest or loader for level ordering. This is the primary and reliable mechanism — do NOT assume other APIs exist unless confirmed by reading GameViewModel after Task 9.

**Navigation back to bookshelf:** `closeGame()` sets `isPlaying = false`, which dismisses the game and returns to `LabyrinthGridView`. To get all the way back to the bookshelf, the implementation must also reset the pack selection state in ContentView (set `selectedPack = nil` or equivalent). This may require a callback passed from ContentView through the grid, or an `@EnvironmentObject` notification pattern. Coordinate with Task 8's ContentView navigation design.
