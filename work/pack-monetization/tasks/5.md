...
- MEMORY.md -- SubscriptionManager uses StoreKit 2, product IDs pattern

### Source code to read before implementation

- `Services/SubscriptionManager.swift` -- the file being modified (97 lines, 3 product IDs, subscription logic)
- `Views/PaywallView.swift` -- caller: uses `subscriptionManager.products`, `subscriptionManager.purchase()`, `subscriptionManager.loadProducts()`, `subscriptionManager.restorePurchases()`. References `yearlyID`, `lifetimeID`, `PlanCard` -- all of which will be updated in Task 7, but the SubscriptionManager API must remain compatible.
- `ViewModels/GameViewModel.swift` -- caller: uses `subscriptionManager.isPremium` only
...
Recommended: Option 1 -- add a deprecated computed property `var products: [Product] { product.map { [$0] } ?? [] }` so the build passes at each task boundary. Task 7 removes it when PaywallView is rewritten.
